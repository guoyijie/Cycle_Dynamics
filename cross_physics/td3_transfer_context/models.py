...
class ContextModel(nn.Module):
    def __init__(self,state_dim, action_dim, history_length, context_dim):
        super(ContextModel,self).__init__()
        self.action_dim = action_dim*history_length
        self.state_dim = state_dim*history_length
...
class ContextForwardModel(nn.Module):
    ...
    def forward(self, state, action, context):
        ...
        state = torch.cat((action, state, context), 1)
...
class MultiAxmodel(nn.Module):
    def __init__(self,opt):
        super(MultiAxmodel,self).__init__()
        self.opt = opt
        self.action_dim = self.opt.action_dim
        self.state_dim = self.opt.state_dim
        self.state_fc = nn.Sequential(
...
class MultiDmodel(nn.Module):
    def __init__(self,state_dim, action_dim, context_dim):
        super(MultiDmodel,self).__init__()
        self.action_dim = action_dim
        self.state_dim = state_dim
        self.context_dim = context_dim
        self.state_fc = nn.Sequential(
