...
from cycle_transfer_contrastive_pure import ActionTransferAgent
	...
	for t in range(int(args.max_timesteps)):
		...
		if t < args.start_timesteps:
			...
			if source_context is not None:
				...
				action = action_transfer_agent.select_action(state, source_context=source_context, target_context_tensor=context_tensor, shared_policy=policy) 
			else:
				action = policy.select_action(np.array(state), context_tensor) 
			if not deterministic:
                                action = (action+np.random.normal(0, max_action * args.expl_noise, size=action_dim)).clip(-max_action, max_action)
		...
		if done:
			...
			if source_context is None or t<args.start_timesteps:
                                idx = data_type.index(context[0])
                                action_transfer_agent.shared_policy_rewards[idx]['timesteps'].append(t+1)
                                action_transfer_agent.shared_policy_rewards[idx]['rewards'].append(episode_reward)
			else:
                                idx = data_type.index(context[0])
                                source_idx = data_type.index(source_context)
                                action_transfer_agent.transfer_rewards[source_idx][idx]['timesteps'].append(t+1)
                                action_transfer_agent.transfer_rewards[source_idx][idx]['rewards'].append(episode_reward)
			...
			writer.add_scalar('train_deterministic/arma%g'%context, int(deterministic), t+1)
			print(
				f"Total T: {t + 1} Episode Num: {episode_num + 1} Episode T: {episode_timesteps} Reward: {episode_reward:.3f} Source: {source_context} Target: {context} Deterministic: {deterministic}")
			# Reset environment
			state, done = env.reset(), False
			cp_obs = np.zeros((args.history_length, env.observation_space.shape[0]))
			cp_act = np.zeros((args.history_length, env.action_space.shape[0]))
			...
			episode_timesteps = 0
			episode_num += 1
			source_context, deterministic = action_transfer_agent.good_to_transfer(target_context=context, threshold=args.threshold, t=t)
...
if __name__ == "__main__":
	...
	parser.add_argument("--reward_delay", default=100, type=int) 
	...
	parser.add_argument("--log_root", default="results_sil_transfer_contrastive_pure_raresil_10000_third/")    
	...
	parser.add_argument("--context_dim", default=5, type=int)
