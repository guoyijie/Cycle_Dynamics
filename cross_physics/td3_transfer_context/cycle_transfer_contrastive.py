...
class ActionTransferAgent(object):
    ...
    def train_agent(self, source_agent, target_agent, source_pool, target_pool, name, writer,t):
        loss_fn = nn.L1Loss()
        epoch_loss, cmp_loss, gan_loss = 0, 0, 0
        for i in range(int(self.pair_n)):
            ...
            writer.add_scalar('%s/cycle_loss'%name, loss_cycle.item(), (t+1)*self.pair_n+i)
            writer.add_scalar('%s/back_loss'%name, loss_back.item(), (t+1)*self.pair_n+i)
            writer.add_scalar('%s/gan_generator_loss'%name, loss_g_gan.item(), (t+1)*self.pair_n+i)
    ...
    def train_ax(self, writer, t, shared_policy):
        ...
        for i, source in enumerate(self.data_type):
            for j, target in enumerate(self.data_type):
                ...
                cycle_loss, back_loss, gan_loss = self.train_agent(self.nested_agents[i], self.nested_agents[j], source_pool=self.pools[i], target_pool=self.pools[j], name=name, writer=writer, t=t)
